...
# Las clases sin clase padre heredan pr default de object no hace falta ponerlo
class EnvironOpt:
    #
    ...
    def change_dir(self, basedir):
        ...
        self.inputdir = basedir + "/data"
        self.bindir = basedir + "/source/python"
        #return / Las funciones retornan sin necesidad de return. Solo
        # es util la sentencia return cuando retornas un valor o cuando
        # necesitas retoran por alguna condición en forma anticipada.
...
class EITdata:
    def __init__(self, environ, inputfile, measure_active_electrodes=1):
        ...
        self.NrMeas, self.NrFrames = np.shape(self.EIT_data_meas)
        ...
        #        self.EIT_data_meas_active_electrodes[3 * k * 16 + 45] = self.EIT_data_meas_active_electrodes[
        #            3 * k * 16 + 47]
        #        self.EIT_data_meas_active_electrodes[3 * k * 16 + 47] = aux
        # Una buena practica
        self.__order_active_electrodes()

    def __order_active_electrodes(self):
        if self.measure_active_electrodes:
            for k in range(self.NrFrames):
                aux = self.EIT_data_meas_active_electrodes[3 * k * 16]
                self.EIT_data_meas_active_electrodes[3 * k * 16] = self.EIT_data_meas_active_electrodes[3 * k * 16 + 2]
                self.EIT_data_meas_active_electrodes[3 * k * 16 + 2] = aux
                aux = self.EIT_data_meas_active_electrodes[3 * k * 16 + 45]
    ...
    def get_normalized_dataframes(self, ref_dataframe):
        ...
        self.EIT_normalized_dif = (self.EIT_data_meas_passive_electrodes.transpose() /
                                   self.EIT_data_meas_passive_electrodes.transpose()[ref_dataframe] - 1).transpose()
    ...
    def estimate_contact_resistances(self, injected_currents):
        ...
        for k in range(16 * self.NrFrames):
            v = abs(self.EIT_data_meas_active_electrodes[3 * k]) + abs(
                self.EIT_data_meas_active_electrodes[3 * k - 1]) / 2
...
class RawImage:
    ...
    #Los estilos de python  usan camecase solo par nombres de clases y el resto (variabels y funciones) notaciónn con _
    # Igualmente si usariamos camelCase para los nombres de los métodos de clases siempre comienzan con minuscula. En
    # este caso sería calcRawImage. Igual siguimos estilos de python.
    ...
    def show_frame(self, frame_number):
        color = self.RawImage[:, frame_number]
        color = np.array([[c, c] for c in color]).ravel()
        plt.tripcolor(self.rec_model.nodes[0], self.rec_model.nodes[1], self.rec_model.tri, facecolors=color)
        plt.show()
        #return
